...
async function notifyBot(endpoint, data) {
    try {
        ...
        console.log(`📨 Уведомление боту:`, result);
        return result;
    } catch (err) {
        console.error('⚠️ Не удалось отправить уведомление боту:', err.message);
        return null;
    }
}
...
app.post('/api/models', async (req, res) => {
    try {
        const { name, program, size, className, yarn, image, isCoat, threading } = req.body;
        ...
        if (isCoat === 'on') {
            for (let i = 0; i < 5; i++) {
                ...
                const partThreading = req.body[`part_threading_${i}`];
...
app.post('/api/tasks/duplicate/:id', async (req, res) => {
    ...
    try {
        ...
        const newParent = await Task.create({
            ...
            partName: null,
            parentTaskId: null,
            threading: original.threading || null
        });
...
app.get('/worker', async (req, res) => {
    try {
        ...
        const pendingShapki = [];
        const pendingCoat = [];
        const inProgressShapki = [];
        const inProgressCoat = [];

        tasks.forEach(task => {
            if (task.status === 'in_progress') {
                if (task.isCoat) {
                    inProgressCoat.push(task);
                } else {
                    inProgressShapki.push(task);
                }
            } else {
                if (task.isCoat) {
                    pendingCoat.push(task);
                } else {
                    pendingShapki.push(task);
                }
            }
        });

        res.render('worker/dashboard', {
            inProgressShapki: inProgressShapki,
            inProgressCoat: inProgressCoat,
            pendingShapki: pendingShapki,
            pendingCoat: pendingCoat,
            shapkiTasks: pendingShapki,
            coatTasks: pendingCoat,
...
app.post('/api/tasks', async (req, res) => {
    ...
    try {
        ...
        if (model.isCoat && model.parts && model.parts.length > 0) {
            ...
            try {
                await notifyBot('/webhook/new-task', { task: coat });
            } catch (err) {
                console.error('⚠️ Ошибка уведомления:', err.message);
            }
            ...
            // ✅ УВЕДОМЛЕНИЕ В TELEGRAM
            try {
                await notifyBot('/webhook/new-task', { task: task });
            } catch (err) {
                console.error('⚠️ Ошибка уведомления:', err.message);
            }
...
app.post('/api/tasks/complete/:taskId', async (req, res) => {
    ...
    try {
        ...
        // ✅ УВЕДОМЛЕНИЕ В TELEGRAM
        try {
            await notifyBot('/webhook/task-completed', { task: task });
        } catch (err) {
            console.error('⚠️ Ошибка уведомления:', err.message);
        }
