...
const model = await Model.create({
    ...
    image: image || null,
    isCoat: isCoat === 'on',
    threading: threading || null  // ✅ ДОБАВЛЕНО
});
...
app.get('/worker', async (req, res) => {
    try {
        // Получаем ВСЕ задания (и pending, и in_progress)
        const tasks = await Task.findAll({
            where: { 
        ...
        const pendingTasks = [];      // для блока "Заказы"
        const inProgressTasks = [];   // для блока "В работе"
        
        // Отдельно для шапок и кофт
        const pendingShapki = [];
        const pendingCoat = [];
        ...
        res.render('worker/dashboard', {
            // Для блока "В работе" (полные карточки)
            inProgressShapki: inProgressShapki,
            inProgressCoat: inProgressCoat,
            // Для блока "Заказы" (компактный список)
...
// ========================================
//  API: ИЗМЕНИТЬ СТАТУС ЗАДАНИЯ (В РАБОТУ / ИЗ РАБОТЫ)
// ========================================
...
app.post('/api/tasks/:id/status', async (req, res) => {
    ...
    try {
        ...
        console.log(`✅ Статус задания #${id} изменён на: ${status}`);
